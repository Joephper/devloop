...
├── git/
│   ├── remote-info.json               # 远程地址、默认分支
│   ├── branches.json                  # 本地/远程分支列表
│   ├── tags.json                      # 标签（倒序）
│   └── permissions.json               # master 推送权限
└── shared-data/
...
```text
需要 Git 元数据 →  先读 .shared_cache/git/*.json
                   ├─ 存在且字段完整 → 直接用
                   └─ 缺失 → git 命令取 → 回写缓存
                   
执行 push/merge/branch 等变更 → 立即刷新对应缓存
```

### 4.4 为什么这样设计

- **不受 git checkout 影响**：`.gitignore` 的文件切分支时不会被 checkout 覆盖
- **所有分支共享同一份**：跨分支的元数据（端口、远程、权限）自然一致
...
git tag -l                                   ← 发布了什么
.shared_cache/git/tags.json                  ← 发布历史
```
